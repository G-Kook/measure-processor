...
class IdvgTransformer(BaseTransformer):
    @classmethod
    ...
    def transform(self):
        ...
        SWEEP_WIDE = SWEEP_LONG.pivot(index=WL_COLS, columns='V_G', values='I_DS', aggregate_function='first')
        FEATURES_LONG = FEATURES_WIDE.unpivot(
            index=WL_COLS, on=[c for c in FEATURES_WIDE.columns if c not in WL_COLS], variable_name='ITEM', value_name='VALUE'
        ).with_columns(
            pl.col('ITEM').cast(pl.Categorical)
        )

        ## UNDERSAMPLE
        SWEEP_LONG = SWEEP_LONG.filter(pl.col('V_G').mul(10).mod(2).eq(0))

        exports = {
            'idvg_sweep': SWEEP_LONG,
            'idvg_features': FEATURES_WIDE,
        }
